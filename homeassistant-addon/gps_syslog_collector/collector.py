...
class _App:
    def __init__(self, assembler, allowed=None):
        self.assembler = assembler
        self.allowed = allowed          # set di hostname ammessi, o None = tutti
        self._warned = set()
    ...
    def handle_line(self, line, src_ip):
        ...
        if not nmea_valid(msg):
            ...
            return
        host = host or src_ip
        # Allowlist: utile quando la porta e' esposta su internet.
        if self.allowed is not None and host not in self.allowed:
            if host not in self._warned:
                log(f"hostname '{host}' non in allowed_hosts: ignorato")
                self._warned.add(host)
            return
        self.assembler.feed(host, msg)
...
def main():
    ...
    allowed_raw = cfg.get("security", "allowed_hosts", fallback="").strip()
    allowed = {h.strip() for h in allowed_raw.split(",") if h.strip()} or None
    app = _App(assembler, allowed)
    if allowed:
        log(f"Allowlist attiva: {sorted(allowed)}")
